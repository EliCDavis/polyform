...
type CreateEquationSubgraphInput struct {
	...
	Equation    string `json:"equation" jsonschema:"an equation of the form 'output = expression', e.g. 'c = sqrt(a^2 + b^2)'. Supports + - * / ^ (numeric-literal exponents only, including negative and 0.5 for sqrt), unary minus, parentheses, the functions sqrt(x), hypot(a,b)/hypotenuse(a,b), min(a,b,...), max(a,b,...), sin(x), cos(x), tan(x), asin(x), acos(x), atan(x), atan2(y,x), radians(deg), degrees(rad) (angles in radians, matching quaternion.FromEulerAngleNode), and the constants pi and e (lowercase only). Every other bare identifier becomes a float64 boundary input, deduplicated by name if it appears more than once. There is no general pow(base,exponent) or abs node in polyform, so those are not supported — you'll get a clear error naming the unsupported piece rather than a silent wrong answer."`
}
