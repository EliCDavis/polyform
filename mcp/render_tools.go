...
func (s *Server) registerRenderTools() {
	mcpsdk.AddTool(s.sdk, &mcpsdk.Tool{
		Name:        "render_preview",
		Description: "Rasterize a fast preview PNG of a gltf Manifest/Model scene using a CPU software rasterizer. Reads a mesh's per-vertex \"Color\" attribute when present (the standard technique for SDF/marched meshes, which have no UVs to texture) - vertex-colored parts render with their real color here, not flat gray; a UV-mapped texture (glTF ColorTexture) still isn't sampled, only the material's flat BaseColorFactor is, for a part that has neither. With no views given, auto-frames a single default angle to the scene's bounding box (fastest, use this for the usual per-checkpoint sanity check). Pass views to render several camera angles and composite them into one grid image in a single call — e.g. front/back/top to catch a part hidden from the default angle, or one zoomed-in view (via target+zoom) to inspect a small detail precisely. Pass exclude (a list of node ids) to leave specific parts out of just this one render, without touching the graph — the fast, non-destructive way to isolate which part is responsible for a visual defect, instead of disconnect/render/reconnect.",
	}, s.renderPreview)
}
