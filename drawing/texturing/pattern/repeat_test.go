...
func TestRepeat_Panic(t *testing.T) {
	assert.PanicsWithError(t, "can not create grid: invalid texture dimensions -1x-1", func() {
		pattern.Repeat(texturing.Texture[float64]{}, vector2.New(1, 1), vector2.New(-1, -1))
	})

	assert.PanicsWithError(t, "can not repeat texture element on a grid 10x-1 times", func() {
		pattern.Repeat(texturing.Texture[float64]{}, vector2.New(10, -1), vector2.New(1, 1))
	})
}
...
func TestRepeat(t *testing.T) {
	...
	for testName, tc := range tests {
		t.Run(testName, func(t *testing.T) {
			assert.Equal(t, tc.expected, pattern.Repeat(tc.element, tc.repeat, tc.targetDimensions))
		})
	}
}
