...
// - Absolute file paths
func (l *StandardBufferLoader) LoadBuffer(uri string) ([]byte, error) {
	// Handle data URIs
...
// StandardImageLoader implements the ImageLoader interface for loading images from
// the file system or data URIs.
...
// - Relative file paths (resolved against BasePath)
// - Absolute file paths
func (l *StandardImageLoader) LoadImage(uri string) (image.Image, error) {
	// Handle data URIs
	if strings.HasPrefix(uri, "data:") {
...
func (l *NoOpImageLoader) LoadImage(uri string) (image.Image, error) {
	return nil, nil
}
